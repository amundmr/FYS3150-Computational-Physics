...
with open('../complete_solar-system/output/jupiter/Earthg.txt', 'r') as f:
    for line in f:
        l = line.split("\t")
        earth_x.append(float(l[0])); earth_y.append(float(l[1])); earth_z.append(float(l[2]))

jupiter_x = []; jupiter_y = []; jupiter_z = []
with open('../complete_solar-system/output/jupiter/Jupiterg.txt', 'r') as f:
    for line in f:
        l = line.split("\t")
...
sun_x = []; sun_y = []; sun_z = []
with open('../complete_solar-system/output/jupiter/Sung.txt', 'r') as f:
    for line in f:
        l = line.split("\t")
...
sc = plt.scatter(earth_x[::Plotskip], earth_y[::Plotskip],s=dotsize, label= "Earth", c = t, cmap = cm)
sc2 = plt.scatter(jupiter_x[::Plotskip], jupiter_y[::Plotskip],s=dotsize, label= "Jupiter", c = t, cmap = cm)
...
plt.title(r"Earth and Jupiter, $M_{Jupiter}=9.54E-4$, $\Delta t = %g yr$"%dt)
#plt.ylim(0,13.5)
#plt.yscale('log')
